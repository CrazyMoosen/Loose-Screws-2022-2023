...
//    final double degPerSec = 150;
    ...
    public RobotPosition(DcMotor fL, DcMotor fR, DcMotor bL, DcMotor bR, double x, double y, Direction direction) {
        super(x, y);
        this.fL = fL;
        this.fR = fR;
        this.bL = bL;
    ...
    private void moveAlongX(double endX) {
        if (endX != x && endX <= 144 && endX >= 0) {
            ...
            if (direction == Direction.Right) {
                moveLinearUsingEncoders(bR, mecanumDrive, magnitude, Math.abs(endX - x));
            }
            if (direction == Direction.Left) {
                moveLinearUsingEncoders(bR, mecanumDrive, !magnitude, Math.abs(endX - x));
            }
            if (direction == Direction.Up) {
                strafeLinearUsingEncoders(bR, mecanumDrive, magnitude, Math.abs(endX - x));
            }
    ...
    private void moveAlongY(double endY) {
        if (endY != y) {
            ...
            if (direction == Direction.Right) {
                strafeLinearUsingEncoders(bR, mecanumDrive, magnitude, dist);
            }
            if (direction == Direction.Left) {
                strafeLinearUsingEncoders(bR, mecanumDrive, !magnitude, dist);
            }
            if (direction == Direction.Up) {
                moveLinearUsingEncoders(bR, mecanumDrive, !magnitude, dist);
            }
            if (direction == Direction.Down) {
                moveLinearUsingEncoders(bR, mecanumDrive, magnitude, dist);
            }
...
//    }
    ...
    public static void strafeLinearUsingEncoders(DcMotor bR, MecanumDrive mecanumDrive, boolean right, double distance) {
        if (right) {
            while (bR.getCurrentPosition() / TICKS_PER_INCH <= distance) {
                if (bR.getCurrentPosition() / TICKS_PER_INCH <= (distance)-0.04) {
