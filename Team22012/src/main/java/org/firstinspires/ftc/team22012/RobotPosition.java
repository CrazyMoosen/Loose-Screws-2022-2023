...
//        }
    ...
    public moveStraight(@NotNull Motor.Encoder bREncoder, @NotNull Motor.Encoder bLEncoder,
                        @NotNull Motor.Encoder fREncoder, @NotNull Motor.Encoder fLEncoder, double distance, double power){
        double offset = 0.05;
        while (bREncoder.getRevolutions() < distance / 12.0D){
            if ((bREncoder.getRevolutions() + fREncoder.getRevolutions()) >
                    (bLEncoder.getRevolutions() + fLEncoder.getRevolutions() + 0.5)){
                fl.set(-(power + offset));
                bl.set(-(power + offset));
                fR.set(power - offset);
                fL.set(power - offset);
            } else if ((bREncoder.getRevolutions() + fREncoder.getRevolutions() + 0.5) <
                    (bLEncoder.getRevolutions() + fLEncoder.getRevolutions())) {
                fl.set(-(power - offset));
                bl.set(-(power - offset));
                fR.set(power + offset);
                fL.set(power + offset);
